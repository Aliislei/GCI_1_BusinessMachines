# [ベースライン] GCI最終課題：I社人事データによる離職予測分析
...
print(f"Percentage of No: {attrition_percentage.get('No', 0):.2f}%")
print(f"Percentage of Yes: {attrition_percentage.get('Yes', 0):.2f}%")
...
# 各カテゴリ変数ごとに 目的変数率を棒グラフで描画
for i, col in enumerate(categorical_cols):
    dataset_temp = dataset.copy()
    dataset_temp['Attrition_num'] = dataset_temp['Attrition'].map({'Yes': 1, 'No': 0})
    mean_values = dataset_temp.groupby(col)['Attrition_num'].mean().sort_values(ascending=False)
    sns.barplot(x=mean_values.index, y=mean_values.values, ax=axes[i])
    axes[i].set_title(f'Attrition Rate by {col}', fontsize=14)
    axes[i].set_xlabel(col, fontsize=12)
    axes[i].set_ylabel('Attrition Rate', fontsize=12)
    axes[i].set_ylim(0, 1)
    axes[i].tick_params(axis='x', rotation=45)
    axes[i].grid(axis='y', linestyle='--', alpha=0.7)
...
def train_cross_validation_models(dataset, features, target_column, n_splits=5):
    ...
    auc = roc_auc_score(test_y, test_pred_proba_mean)
    print(f"\nAverage Real AUC: {round(auc, 4)}")
